...
class Factory {
    ...
    // the for loops will create our empty slots, so that we can later generate random cards into each one.
    ...
    // We are passing in the slots array because we don’t want to add a card that already exists in a column. 
    // So, we will be able to randomize the cards, but if it exists in the array already, it will generate 
    // a new card in it’s place.
